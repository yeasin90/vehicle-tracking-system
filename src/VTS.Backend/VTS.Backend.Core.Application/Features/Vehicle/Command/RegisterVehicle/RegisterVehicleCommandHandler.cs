...
{
    public class RegisterVehicleCommandHandler : IRequestHandler<RegisterVehicleCommand, RegisterVehicleDto>
    {
        ...
        public async Task<RegisterVehicleDto> Handle(RegisterVehicleCommand request, CancellationToken cancellationToken)
        {
            var entity = _mapper.Map<Domain.Entities.Vehicle>(request);
            var savedEntity = await _vehicleRepository.AddAsync(entity);
            return _mapper.Map<RegisterVehicleDto>(savedEntity);
        }
    }
}
